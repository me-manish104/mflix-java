...
public class CommentDao extends AbstractMFlixDao {
    ...
    private MongoCollection<Document> criticCollection;
    ...
    public CommentDao(
        ...
        this.criticCollection =
                db.getCollection(COMMENT_COLLECTION).withCodecRegistry(pojoCodecRegistry);
    }
    ...
    public boolean updateComment(String commentId, String text, String email) {
        ...
        log.info("=========update receieve for email {} id {} text {}",email,commentId,text);
        Bson query = Filters.and(new Document("_id",new ObjectId(commentId)), new Document("email",email));
        return commentCollection.updateOne(query,Updates.set("text",text)).getModifiedCount()>0;
    }
    ...
    public List<Critic> mostActiveCommenters() {
        ...
        // // Critic objects.
        ArrayList<Document> result = criticCollection.aggregate(Arrays.asList(Aggregates.group("$email", Accumulators.sum("count", 1L)),
                Aggregates.sort(Indexes.descending("count")), Aggregates.limit(20))).into(new ArrayList<>());
        for (Document doc : result) {
            mostActive.add(new Critic((String)doc.get("email"),(((Long)doc.get("count"))).intValue()));
        }
         return mostActive;
    }
}

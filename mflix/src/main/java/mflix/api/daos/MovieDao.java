...
public class MovieDao extends AbstractMFlixDao {
    ...
    public Document getMovie(String movieId) {
        ...
        Aggregates.match(Filters.eq("_id", new ObjectId(movieId)));
        pipeline = Arrays.asList(match,
                Aggregates.lookup("comments",
                        Arrays.asList(new Variable("id", "$_id")),
                        Arrays.asList(Aggregates.match(Document.parse("{ $expr: { $eq: ['$movie_id', '$$id'] } }"))
                                , Aggregates.sort(new Document("date", -1))), "comments"));
    ...
    public List<Document> getMoviesByCountry(String... country) {
        ...
        List<Document> movies = new ArrayList<>();
        moviesCollection.find(Filters.all("countries", country)).projection(Projections.fields(Projections.include("title"))).into(movies);
